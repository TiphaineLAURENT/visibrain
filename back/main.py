...
@app.get("/videos/")
async def videos(
    twitch: Annotated[Twitch, Depends(twitch)], game_name: str
) -> list[Video]:
    ...
    logger.debug(
        "Fetching recent videos from mongodb for game %s (%s)", game.id, game.name
    )
    saved_videos = list(
        videos_repository.find_by(
            {
                "saved_at": {
                    "$gte": datetime.datetime.now() - datetime.timedelta(minutes=2)
                },
                "game_id": game.id,
            }
        )
    ...
    api_videos = [
        Video.model_validate({**video.to_dict(), "game_id": game.id})
        async for video in twitch.get_videos(game_id=game.id)
        if "404" not in video.thumbnail_url
    ]
    logger.debug("Found %s videos from Twitch API", len(api_videos))
